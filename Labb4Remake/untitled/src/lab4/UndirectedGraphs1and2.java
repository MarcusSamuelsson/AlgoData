...
Problem approached: Lab 4 - Graphs, problem 1,2
...
class UndirectedGrapth1and2 {
    ...
    private void addNodes(String filename) {
        ...
        while (!in.isEmpty()) {
            ...
            for (int i = 0; i < a.length; i++) {
                if (i % 2 == 0) {   //Node is on the left side in the line
                    if (!st.contains(a[i]))
                        st.put(a[i], 1, a[i + 1]);
                    else
                        st.put(a[i], (st.get(a[i]) + 1), a[i + 1]);
                } else {    //Node is on the right side of the line
                    if (!st.contains(a[i]))
                        st.put(a[i], 1, a[i - 1]);
                    else
                        st.put(a[i], (st.get(a[i]) + 1), a[i - 1]);
    ...
    public static void main(String[] args) {
        UndirectedGrapth1and2 ug = new UndirectedGrapth1and2();
